...
[NOTE]
====
SUPERSEDED 2026-09-15. This audit predates PR #93 — the engine now has 318
passing tests, so "Unit tests: NONE (0 test files found)" below is false — and
predates the FOT-R-22 retire, which deleted the nine migration-observer,
batch-reviewer, report-generator and GenStage-pipeline modules still named in
the file list below. Re-run the audit before acting on any figure here.

VeriSimDB itself is NOT retired: it remains a live estate integration
(`.machine_readable/integrations/verisimdb.a2ml`, gated by `dogfood-gate.yml`).
Only the two FoT-internal writers went with the pipeline.
====
...
feedback_a_tron/submitter.ex — no tests - feedback_a_tron/audit_log.ex —
no tests - feedback_a_tron/cli.ex — no tests -
...
feedback_a_tron/channels/discourse.ex — no tests - Plus ~14 more modules
- 3 Idris2 ABI files — no tests - 1 Julia file — no tests
...
* [ ] Security (MCP tool injection, network verifier bypass, NNTP
injection, audit log tampering)
* [ ] Performance (batch review throughput, pipeline backpressure,
VeriSimDB write latency)
* [ ] Concurrency (GenStage pipeline, concurrent submissions, supervisor
restart)
* [ ] Error handling (VeriSimDB unavailable, network failures, malformed
feedback)
* [ ] Accessibility (N/A)
...
* *HIGH* — 34 Elixir source files with ZERO tests. This is a feedback
processing system with MCP integration, GenStage pipeline, multi-channel
input (NNTP, Discourse), VeriSimDB integration, and audit logging. Not a
single module has a test file. The pipeline supervisor/producer/consumer
pattern especially needs testing for correctness under load and failure
conditions.
