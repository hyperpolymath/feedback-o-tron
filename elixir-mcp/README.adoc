...
* *Datalog Analysis* — Derive relationships, detect patterns, find
regressions
...
|===
|Rule |Description
|`+related(X, Y, Reason)+` |Issues X and Y are related (mentions,
same_component, same_label)

|`+state_sync_issue(X)+` |Issue X is about state synchronization

|`+regression(X)+` |Issue X was closed then reopened

|`+fix_caused_regression(PR, Fixed, Broke)+` |PR fixed one issue but
caused another
